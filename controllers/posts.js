...
const postsController = {
  create: async (req, res) => {
    ...
    usersFollowing.forEach(( item ) => {
      console.log(userSocketIdMap ,item.user_id) ;
  ...
  addComment : async( req , res ) => {
    try {
      ...
      const {id : userId } = jwt.verify(req.cookies.token , process.env.JWT_SECRET ) ;
  ...
  getAllComments : async ( req , res) => {
    try {
      ...
      const allCommentsMap = allComments.reduce(( first , second ) => {
          const sorted = second.comments.sort(( a , b ) => b.createdAt - a.createdAt ) ;
